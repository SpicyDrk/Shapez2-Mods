...
/// <summary>
/// Smoke check for the HelloWorld mod's <c>manifest.json</c>. Returns exit code
/// 0 if all assertions pass, 1 otherwise. Designed to be runnable from CI, from
/// `dotnet run --project Tests/HelloWorld.Tests/`, or from a Rider run config.
///
/// We avoided xunit here because the .NET 9/10 SDKs surface a
/// Microsoft.TestPlatform.CoreUtilities-15 resolution failure that blocks
/// VSTest execution. A plain console app with explicit checks is simpler,
/// has zero external test dependencies, and still gives a clear pass/fail
/// signal.
/// </summary>
public static class Program
{
    private const string ModName = "HelloWorld";
