...
namespace CustomAsteroids
{
    ...
    /// code is captured into <see cref="CustomAsteroidUiState"/> (Task 3 turns that into a
    ...
    /// </summary>
    internal sealed class CustomAsteroidAuthoringDialog
    {
        private readonly CustomAsteroidUiState _ui;
        private readonly ILogger _logger;

        public CustomAsteroidAuthoringDialog(CustomAsteroidUiState ui, ILogger logger)
        {
            _ui = ui;
            _logger = logger;
        }
        ...
        public void Open()
        {
            ...
            {
                _logger.Warning?.Log("[CustomAsteroids:ui] authoring dialog requested but the dialog stack isn't captured yet.");
                return;
            }
...
#pragma warning restore CS0618
                IText title = new RawText("Custom Asteroid");
                ...
                IText defaultValue = new RawText(_ui.AuthoredCode ?? CustomAsteroidUiState.DefaultShapeCode);
            ...
            {
                _logger.Error?.Log($"[CustomAsteroids:ui] failed to open authoring dialog (non-fatal): {ex}");
            }
        }
        ...
        private void OnConfirmed(string code)
        {
            try
            {
                if (CanonicalShapeResolver.TryResolve(code, out ShapeDefinition shape, out string diag))
                {
                    ...
                    _logger.Info?.Log(
                        $"[CustomAsteroids:ui] authored shape accepted: '{_ui.AuthoredCode}' ({diag}). " +
                        "Placement armed — left-click the space map to place, Esc/right-click to cancel.");
                }
                else
                {
                    _ui.AuthoredShape = null;
                    _logger.Warning?.Log($"[CustomAsteroids:ui] rejected shape code '{code}' ({diag}).");
                    ShowError(code, diag);
                }
            }
            catch (Exception ex)
            {
                _logger.Error?.Log($"[CustomAsteroids:ui] confirm handler threw (non-fatal): {ex}");
            }
        }
...
#pragma warning restore CS0618
            ...
            {
                _logger.Error?.Log($"[CustomAsteroids:ui] failed to show invalid-code dialog (non-fatal): {ex}");
            }
        }
    }
}
