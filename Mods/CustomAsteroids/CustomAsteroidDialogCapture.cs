...
namespace CustomAsteroids
{
    ...
    /// </summary>
    internal sealed class CustomAsteroidDialogCapture : IDisposable
    {
        private readonly Hook _hook;
        private readonly CustomAsteroidUiState _ui;
        private readonly ILogger _logger;

        public CustomAsteroidDialogCapture(CustomAsteroidUiState ui, ILogger logger)
        {
            _ui = ui;
            _logger = logger;
            ...
            ConstructorInfo ctor = typeof(HUDDialogStack).GetConstructor(
                ...
                ?? throw new InvalidOperationException(
                    "CustomAsteroids: failed to find HUDDialogStack(Transform, IPrefabInstanceProvider, ILogger) ctor.");

            CtorDelegate detour = CtorPostfix;
            _hook = new Hook(ctor, detour);

            logger.Info?.Log("[CustomAsteroids:ui] dialog-stack capture hook installed on HUDDialogStack ctor.");
        }
            ...
            ILogger logger)
        {
            ...
            _logger.Info?.Log("[CustomAsteroids:ui] captured IHUDDialogStack (authoring dialog is now available).");
        }
    }
}
